...
예를 들면, `FileInputStream`의 `close()`나 `Connection`의 `close()`등이 있다.    

 
하지만, `try 문` 안에다 작성하기에는 실행되지 않고 넘어갈 가능성도 존재하고  
`catch 문` 안에다 넣어도 예외가 발생하지 않으면 실행하지 않게 된다.   
그렇다면 `try-catch 문` 블록 밖에다 기술하는 수 밖에 없는데 
