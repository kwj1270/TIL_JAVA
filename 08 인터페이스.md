...
이 부분에 관해서는 [클린코드]()를 통해 알 수 있으며   
간략히 말하면 함수가 한 가지 기능만 수행하므로 재사용성이 좋은 코드를 작성했고   
단순한 값을 출력하는 것이 아닌 로직을 통해 얻은 값을 가졌으니   
단순 getter/setter 형식의 빈약한 클래스가 내부 데이터가 숨겨진 인터페이스가 되었다.   

 
여기서 몇 가지 의문점이 드는 사람이 있을 수 있다.         
...
**인터페이스에서 private 메서드를 사용하는 4가지 규칙**
