...
* 정확한 내부동작은 모르지만 `Stream<E>`을 구현한 클래스의 `filter()`는       
* `Predicate<T>`의 `boolaen test(T t)`메서드를 통해    
* `boolaen test(T t)`가 정의된 로직에서 `true`를 반환하는 요소만 남길 것이다.
