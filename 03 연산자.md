...
> if-else 와 달리, 더 많은 조건의 결과를 표현하기 위해 사용되며      
> 가독성이 더 좋고, 컴파일러가 최적화를 쉽게 할 수 있어 속도 또한 빠른 편이다.   
> switch 표현식에서는 wrapper 허용하지만, case 에는 wrapper 를 허용하지 않는다.
> float, double과 같은 같은 부동 소수점 리터럴을 비교하는 것은 정확하지 않을 수 있어 허용하지 않는다.
