...
|Annotation|description|
|:--------:|:----------|
|@DisplayName|어떤 테스트인지 **테스트 이름을 보다 쉽게 표현**할 수 있는 방법을 제공하는 어노테이션.<br>@DisplayNameGeneration 보다 우선 순위가 높다.|
|@DisplayNameGeneration|Method와 Class 레퍼런스를 사용해서 **테스트 이름을 표기하는 방법을 설정.**<br>클래스에 적용하면 모든 메서드에 적용된다.<br>쉽게 설명하면 테스트시에 콘솔창에 표현되는 메서드 이름을 전략에 맞게끔 표시하는데 사용된다.<br>기본 구현체로 ReplaceUnderscores 제공|
