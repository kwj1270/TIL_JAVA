...
public class GitHubApp {
    private final String GITHUB_ACCESS_TOKEN = "64f7f677b6a34d16a6ae994ba39c7d6cf050ff22";
    ...
    private Participant findParticipant(List<Participant> participants, String userName) {
        if (isNewUser(participants, userName)) {
            ...
            return participants.stream().filter(p -> p.userName.equals(userName)).findFirst().get();
        }
    }

    private boolean isNewUser(List<Participant> participants, String userName) {
        return !(participants.stream().filter(p -> p.userName.equals(userName)).findFirst().isPresent());
    }
    ...
    private String checkMark(List<GHIssue> list, Participant participant) {
        ...
        for (int index = 1; index <= list.size(); index++) {
            result += "|";
            if (participant.homeworkResult.containsKey(index)) {
                result += " [x] ";
                continue;
            }
            result += " [] ";
