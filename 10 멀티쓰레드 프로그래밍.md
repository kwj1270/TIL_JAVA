...
**필자의 생각 :**   
그렇다면 우선순위가 10이상이면 어떻게 되지? : [참고바람](https://stackoverflow.com/questions/21794411/thread-priority-for-more-than-10-threads-in-java)     
우선 결론만 말하자면, 불가능하다고 말하고 있다.    
애초에, 10개의 쓰레드를 동시에 사용한다는 것은 잘못된 설계를 했을 가능성이 높다.       
...
또한, 여기서 배운 내용 중 하나로 **쓰레드의 우선 순위는 강력한 보장을 제공하지 않는다.**       
즉, GC 처럼 JVM에게 힌트를 주는 것이지 100% 절대적이라고는 말할 수 없다는 것이다.       
