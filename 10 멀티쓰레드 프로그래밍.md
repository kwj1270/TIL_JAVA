...
중첩을 피할 수 없으면 **`synchronized`의 인스턴스를 하나로 통일하는 방법이 있다.**   
...
//`static 변수` 하나 만들어서 `lock`용 인스턴스로 사용하는 방법이 있다.                
이 방법은 좀 세밀한 동기화가 되지 않기 때문에 성능이 많이 떨어질 수 있다.                
하지만, 성능은 떨어질지 언정 데드락은 확실히 피할 수 있다.                      
