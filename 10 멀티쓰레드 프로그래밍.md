...
/*
java.lang.Exception
      at com.UserThread.throwException(Main.java:17)
      at com.UserThread.run(Main.java:12)
*/
```
쓰레드를 생성하고 `start()`를 호출해 '호출스택'을 생성하고 `run()`동작을 취했다.   
이 과정에서 고의로 `Exception` 예외가 발생하도록 코드를 구성했다.  
예외처리로 예외가 발생한 당시의 호출스택을 출력하도록 했는데,   
`run`, `throwException`이 출력된 것을 알 수 있다.   
   
![call stack](https://user-images.githubusercontent.com/50267433/105427524-b5330d80-5c90-11eb-8a89-1604aa16adaa.PNG)   
