...
|FileReader(File file)|실제 파일과 연결된 File 인스턴스를 통해 `FileReader`를 생성한다.|
|FileReader(FileDescriptor fd)|파일 디스크립터로 `FileReader` 생성한다.|
|FileOutputStream(String name)|특정 파일이름(name)을 가진 실제 파일과의 연결된 FileOutputStream을 생성한다.|
|FileOutputStream(String name, boolean append)|지정된 파일이름(name)을 가진 실제 파일과 연결된 FileOutputStream을 생성한다.<br>두번째 인자인 append를 true로 하면, 출력 시 기존의 파일 내용의 마지막에 덧붙이며, false이면 덮어쓴다.|
|FileOutputStream(File file)|실제 파일과 연결된 File 인스턴스를 통해 FileOutputStream(String name)을 생성한다|
|FileOutputStream(File file, boolean append)|실제 파일과 연결된 File 인스턴스를 통해 FileOutputStream(File file, boolean append)을 생성한다.<br>두번째 인자인 append를 true로 하면, 출력 시 기존의 파일 내용의 마지막에 덧붙이며, false이면 덮어쓴다.|
