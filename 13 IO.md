...
하지만, 자주 압출력을 하는 경우 이러한 작업은 매우 귀찮고 번거로운 일이다.              
     
자바에서는 이러한 번거로움을 해소하기 위해 표준 입출력(표준 스트림)을 제공한다.      

하지만, 표준 입출력(표준 스트림) 같은 경우는             
자바 어플리케이션의 실행과 동시에 사용할 수 있게 자동적으로 생성되기 때문에                
...
표준 입출력 3가지 입출력 스트림은    
`System` 클래스에 존재하는 `static 메서드`이며 참조 대상은 아래와 같다.   
...
우리가 주요 사용하는 `PrintStream` 의 내부 메서드인        
