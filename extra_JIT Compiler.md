...
그래서 `스프링 네이티브`는 `GraalVM`을 사용해서 이러한 문제를 낮추었다고 하네요!         
...
클라이언트 컴파일러를 사용 할 때 JVM은 컴파일 스레드 한 개로 시작합니다.    
서버 컴파일러는 두 개의 스레드로 시작합니다.
...
루프(반복)가 OSR을 통해 컴파일되었다면 루프가 다음 반복될 때 바로 컴파일된 코드를 실행합니다.
