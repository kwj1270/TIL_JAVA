...
이로인해 다른 패키지나 클래스에서 enum 타입에 접근해서 동적으로 값을 지정할 수 없다.          



Enum은 서로 연관된 고정된 상수의 집합이기 때문입니다. 즉, 외부에서 객체를 생성하지 못하게 막으며, 따라서 생성자를 통해 값을 초기화하거나 동적으로 변환하지 못하게 되는것입니다.
...
```
* oracle에서 명시한 대로 메서드를 정의해보면 위와 같은 코드를 작성해야한다.    
   
결과적으로 enum 에 `{}`를 하는 것은 현재 enum을 상속받는        
익명 클래스를 선언하는 것으로 정리하면 좋을 것 같다.         
...
* [리플렉션을 통해 싱글턴을 깨트릴 수도 없다.](http://wiki.hash.kr/index.php/%EC%8B%B1%EA%B8%80%ED%86%A4%ED%8C%A8%ED%84%B4#reflection.EC.9C.BC.EB.A1.9C_.EC.8B.B1.EA.B8.80.ED.86.A4_.EA.B9.A8.EA.B8.B0)     
