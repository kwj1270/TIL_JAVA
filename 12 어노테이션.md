...
```
* 클래스 파일로 변환되면서 인터페이스가 사라진 것을 알 수 있다.      
...
마커 인터페이스로부터 차근차근 벨류를 넣으면서 정의하는 방법을 기술해보겠다.      
...
`@Repeatable()` 선언된 인터페이스를 다른 인터페이스에서 배열로 사용하고 있어야 한다.    
...
```
위 코드를 보면 `@Colors`를 통해 `@Color`를 중복해서 정의할 수 있다.      
다시 한번 말하지만, **`@Colors`를 통해 `@Color`를 중복해서 정의할 수 있다.**       
  
정확히 말하면, **`@Color`를 위해 `@Colors`를 정의한 것이 아니다.**         
**`@Colors`를 위해 조그만하게 나눈 어노테이션 `@Color`를 선언한 것이다.**           
...
위 코드를 보면 `@Colors`가 아닌, `@Color`를 중복해서 정의할 수 있다.       
이를 통해 알 수 있는 점은 `@Colors`는 Color를 중복해서 정의할 수 있도록 마 
...
Lombok은 어노테이션 프로세서를 제공하여       
**표준적으로 작성해야 할 코드를 개발자 대신 생성해주는 라이브러리이다.**   
        
        
        
어노테이션 프로세서는        
인풋으로 자바 코드(또는 컴파일된 바이트 코드)를 받아서        
아웃풋으로 파일(보통 .java 파일)을 생성합니다.           

여러분이 자바 코드를 생성할 수 있음을 뜻합니다. 
생성된 자바코드는 .java 파일 입니다. 
하지만 이미 존재하는 자바 파일을 수정해서 메서드를 추가하는 것은 할 수 없습니다.
생성된 자바 파일은 javac 에 의해 컴파일 될 것입니다.
