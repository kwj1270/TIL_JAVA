...
# 메타 어노테이션
메타 어노테이션이란 다른 어노테이션에 적용되기 위한 어노테이션을 의미한다.  
...
@interface UserCustomAnnotation { }

@UserCustomAnnotation  // 사용
class Test {
    private final int age= 10;
    public void run(){}
}
...
```
`@Target(ElementType.TYPE)`는 `class`, `interface`, `enum`에서 사용할 수 있다.   
...
class Test {
    ...
    @UserCustomAnnotation           // 사용 
    private Direction direction = Direction12.EAST;
...
```
`@Target(ElementType.FIELD)`는 변수에 사용할 수 있다. (static/final 상관 없다.)         
