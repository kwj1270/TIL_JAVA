...
* 해시 테이블을 내가 지정한 범위로 지정할 수 있다.         
* 하지만, 범위가 존재하므로 겹치는 객체마다 해시코드가 나올 수 있다.         
* 해시코드가 겹칠 경우 LinkedList 기반으로 앞 값에 뒤이어 붙는다.       
...
**이거 내일 아침에 정리**   
`== `는 똑같이 주소값을 비교함
대신 `Hash<Key,Value>`에서 key는 hashcode()반환값 사용 
hashcode()의 디폴트로는 주소값을 사용한다.   
      
하지만, 만약 `equals()`를 바꾸었는데 `hashCode()`안바꿨다면?      
객체1, 객체2는 주소값이 다르지만, 내부 값이 같아서 같다고 처리하고자 한다.   
하지만, `hashCode()`는 주소값을 처리하므로 결국 Hash에서 key를 찾을 수 없게 된다.   
이를 해시충돌이라고 부른다.   
   
반대로, `hashCode()`를 바꾸었는데 `equals()`를 바꾸지 않았다면,     
Hash에서 key를 찾을 수 있지만, 동등 내부 값이 같아서 같다고 처리하지 못한다.   
...
[hash code](https://velog.io/@sonypark/Java-equals-hascode-%EB%A9%94%EC%84%9C%EB%93%9C%EB%8A%94-%EC%96%B8%EC%A0%9C-%EC%9E%AC%EC%A0%95%EC%9D%98%ED%95%B4%EC%95%BC-%ED%95%A0%EA%B9%8C)    
