...
* 하지만, 이는 참조 인스턴스에 대한 내용만 출력하는 단순한 `공통 로직`이다.      
* 만약 객체마다 로직을 아에 다르게 처리해야한다면 어떻게 구성해야할까?   
...
* `SmartPhone`을 구현하는 새로운 `ConcreateClass`가 추가되면 메서드를 수정해야한다.             
* 각 조건문에 공통된 로직을 넣으면 코드의 길이가 개수만큼 기하급수적으로 늘어난다.           
* 즉, `SmartPhone`을 구현하는 클래스 종류 갯수에 따라 메서드의 길이가 기하 급수적으로 길어진다.           
* 결과적으로, 코드의 길이가 길어지고 가독성이 안좋아지면서 OCP의 원칙을 깨뜨리는 나쁜 코드가 된다.        
...
* 그렇다고 미리 명시적 형변환을 하기에는 `if 문`이 단순히 밖으로 나온 것밖에 되지 않다.   
