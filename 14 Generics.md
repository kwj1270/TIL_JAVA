...
우선, 오해와 논란의 소지가 많을 것이고 **바이트 코드상으로도 절대 아니지만,**                 
이해를 돕기 위해서 `<T extends Some>`으로 인해 바운디드 타입으로 선언된 `T` 자체가           
`Some`의 하위 클래스인 `SSome` 으로 변환되었다고 가정하는 것이 도움이 될 것이다.        
다시 한번, 말하지만 이해를 돕기 위한 것이지 코드상으로는 절대 타입이 바뀐 것은 아니다.      
