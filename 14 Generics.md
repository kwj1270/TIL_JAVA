...
`재내릭에 선언된 클래스 타입`으로 변현되거나 `Object 타입`으로 변환을 시키는 방법이 있다.                    


`Law 타입`은 `<>`없는 클래스 즉, 제네릭 이전 클래스 자체를 의미한다.  
예를 들면 `HashMap<K,V>`같은 경우 `HashMap`이 `Law 타입`이다.    
         
`<T>`와 같이 일반적인 제네릭 타입 매개변수를 사용할 경우 `Object`타입으로 변형이 되고  
`<T extends SomeThing>`과 같은 바운디드 타입에서는 `SomeThing`형태로 변형이 된다.      


앞서, primitve 타입은 제네릭을 사용하지 못한다고 기술했다.        
