...
* 즉, 기계어마다 실행하는 명령어가 달라서 각 OS에 맞게끔 코드를 작성해야 했다.          
* JVM은 자바 코드가 기계어로 변환되기 전에 중간 코드인 바이트 코드를 만든다.        
* JVM에서는 OS에 맞게끔 바이트 코드를 알맞는 기계어로 변환을 해주는 작업을 진행한다.      
* 그렇기 때문에 JVM은 JAVA가 OS에 종속되지 않고 재사용할 수 있도록 도와준다.      
